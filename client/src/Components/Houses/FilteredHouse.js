...
const FilteredHouses = ({ houses, houseDetails, searchHouse, likePost, editPost }) => {
        ...
        {searchHouse.map(house => (
            <Grid item xs={12} sm={6} md={3} lg={3} key={house._id} className={classes.item}>
                <Card className={classes.card} elevation={3}>
                    ...
                    <Divider/>
                    <CardActions>
                        <IconButton onClick={() => likePost(house._id)}><FavoriteIcon color={ JSON.parse(localStorage.getItem('profile')) && house.author === JSON.parse(localStorage.getItem('profile'))._id ? 'secondary' :'primary'}/></IconButton>
